...
## ⚠️ 核心关卡 (The Checkpoint) - 路径与模式确认

**在调用任何生图脚本之前，你必须集齐四大核心参数：`画面描述`、`风格倾向`、`绝对保存路径`、`生图模式/横纵比`。**
...
**模式规则：**
- `fast`：快速模式，默认使用 `gemini-3.1-flash-image-preview`。当用户未明确要求时，**必须默认使用该模式**。
- `quality`：高质量模式，使用当前已实现的 `gemini-3-pro-image-preview`。仅当用户明确要求高质量、终稿、超清、精修、商用成图时使用。

**通道规则：**
- `provider=auto`：默认模式。优先使用 `APIMart`，如果在**提交前**因网络异常、5xx、429、无效响应、未拿到 `task_id` 等原因失败，则自动切换到 `OpenRouter`。
- `provider=apimart`：仅使用 APIMart，不做备用切换。
- `provider=openrouter`：仅使用 OpenRouter。
- **重要边界**：如果 APIMart 已经返回了 `task_id`，后续轮询失败、超时或任务失败时，**禁止自动切换到 OpenRouter**，必须按资金保护规则汇报给用户，避免双重扣费。

**横纵比规则：**
- 推荐优先使用 `--aspect_ratio`，语义比 `--size` 更清晰。
- 脚本仍兼容 `--size` 旧写法，但新文档与新调用一律优先使用 `--aspect_ratio`。
- 如果用户未指定比例，按场景自动推断：
  - 表情包、头像、图标 -> `1:1`
  - 手机封面、竖版海报 -> `9:16`
  - PPT 配图、Banner、普通横版插图 -> `16:9`

**分辨率规则：**
- 默认使用 `1K`，这是标准默认值，也是**未明确要求时唯一允许的默认分辨率**。
- 只有当用户明确要求更高清、印刷级、超清细节、大图裁切、商用终稿时，才提升到 `2K` 或 `4K`。
- 不要因为画面复杂、风格精细、使用 `quality` 模式，就自动提高分辨率。
...
- `--mode`: 生图模式。`fast` 为默认快速模式，`quality` 为高质量模式。
- `--aspect_ratio`: 横纵比，如 `16:9`, `1:1`, `4:3`, `9:16`, `21:9`, `1:4`, `8:1`。默认按场景自动推断；未命中时默认 `16:9`。
- `--n`: 每个提示词生成图片的数量，默认为 1。如果用户需要针对同一个提示词同时生成多张图，可以指定此参数。
- `--resolution`: 分辨率，默认为 `1K`，可选 `0.5K` / `1K` / `2K` / `4K`。**除非用户明确要求，否则固定使用 `1K`。** 高分辨率成本更高。
- `--size`: 兼容旧命令的别名，不再作为首选写法。
...
## 依赖说明
- **API 通道**：默认通道为 APIMart Gemini Image，快速模式使用 `gemini-3.1-flash-image-preview`，高质量模式使用 `gemini-3-pro-image-preview`。备用通道为 OpenRouter 图像生成接口。
- **凭证管理**：底层脚本依赖 `secrets-vault` 技能自动获取 `apimart_image` 与 `openrouter_image` 两个命名空间的鉴权信息。

...
**目标**：通过互动思辨，确定核心主旨、宏观结构，并逐模块敲定呈现方式与配图方案，最终生成详细大纲。
...
**【格式规范：AI 生图】**
```markdown
![[AI生图] <这里写纯画面的英文提示词描述>](Materials/Medias/images/<语义化英文命名>.png)
```
*(注：`article-illustrator` 会提取 alt 中的描述作为生图 prompt)*
...
**预计字数**：150-200字
**配图方案**：
![[AI生图] A frustrated programmer sitting in a dark room illuminated only by multiple glowing monitor screens showing complex error logs, cyberpunk style](Materials/Medias/images/frustrated_programmer.png)
...
### 步骤3.1.5：小红书配图策划（XHS 专属）⭐⭐⭐

> **触发条件**：目标平台包含 `xhs`（小红书）时必须执行，其他平台跳过本步骤。

**目的**：为小红书图文笔记制定完整的配图策划方案，指导后续图片准备
...
步骤 F：生成配图策划文档
  输出到 Output/_drafts/03_xhs_image_plan.md
  包含：
  - 模板类型 & 笔记类型说明
  - 建议图片总数
  - 逐图详细方案（每张图独立章节）
  - 每张图的 AI 生图提示词（可直接复制使用）
```

**输出格式**（`03_xhs_image_plan.md`）：
...
├─ 即刻（jike）
└─ Twitter/X
→ 规则：正文不嵌入图片，图片顺序由 image_plan.md 管理
```
...
- **图片处理指令**（⭐⭐⭐ 重要，按平台格式区分）：
  ...
  **【图文混排型平台】**（wechat / zhihu / bilibili / linkedin）：
  ...
  - ⚠️ **如果占位符是 `[待截图]`**：务必保留该图片占位及其下方的 `> 💡 **截图指引**：...` 引用块，不能删除。
  - 标准格式：
    ```markdown
    ![[AI生图] 英文提示词](Medias/images/01-preview.png)
    或
...
**【图文分离型平台】**（强制遵守）：
1. **纯文字**：正文中不出现 `![](...)` 图片语法
2. **图文对齐**：确保文案内容与 image_plan.md 中的图片顺序逻辑一致
3. **image_plan.md 为准**：图片的选择、排序、封面指定全部由 image_plan.md 管理
...
**标准 Markdown 格式**（必须遵守，注意区分生图与截图占位符）：

```markdown
![[AI生图] 英文提示词描述](Medias/images/concept.png)
或
![[待截图] 中文场景描述](Medias/images/screenshot.png)
> 💡 **截图指引**：这里写明确的截图要求...
```

> ⚠️ 图注只写在 alt 文本中，**不要**另起 `*▲ ...*` 行（markdown-to-wechat 会自动将 alt 渲染为 figcaption，另写会重复）
> ⚠️ 如果是 `[待截图]`，不要擅自删除下方的 `> 💡 截图指引：...` 引用块，这是留给作者阅读的，直到阶段6发布时才会清理。
...
在开始使用这个工具之前，我想先带你看看它的核心理念：数据在安全的环境下流转。

![[AI生图] Data packets flowing in a glowing secure tunnel, cyber style](Medias/images/01-concept.png)

看到了吗？这个设计的核心理念就是：让你专注于核心功能，不被复杂的选项干扰。

接下来，我会教你如何配置工具栏。

![[待截图] 拖拽配置工具栏面板](Medias/images/02-toolbar-config.png)
> 💡 **截图指引**：在此处截取系统“工具栏设置”界面的图，并用红框标出“拖拽区域”。
...
3. 根据平台内容格式处理 article.md 中的图片：
   ...
   ├─ 更新为相对路径：images/xxx.png（去掉 Medias/ 前缀）
   ├─ **清理截图指引（⭐新增步骤）**：找到所有的 `![[待截图]...` 占位符及其下方的 `> 💡 **截图指引**：...` 引用块，将下方的 `> 💡` 引用块直接删除，只保留 `![...]` 占位行，确保正式文章干净整洁。（注意：仅图文混排平台执行清理）
   └─ 图片保留在文章正文中

   【图文分离型】（xhs/instagram/douyin/weibo/jike/twitter）：
   ├─ article.md 中不包含图片语法（纯文字）
...
# article.md 中（阶段6输出，指引被清理，路径相对化）
...
for each 图片引用 in draft.md:
  原引用：![[类型] 说明](Medias/images/subfolder/01.png)
  ↓
  1. 复制文件：
  ...
  2. 更新引用：
     将路径：Medias/images/subfolder/01.png
     替换为：images/subfolder/01.png
     
  3. 清理引用块：
     找到图片下方的 `> 💡 **截图指引**：...` 行（如果有），将其从最终内容中删除。
```
...
#### 1.5. image_plan.md（小红书配图策划文档）⭐ XHS 专属

> **仅当目标平台为小红书时生成**

**来源**：从阶段3步骤3.1.5生成的 `Output/_drafts/03_xhs_image_plan.md` 提取，并根据最终稿内容进行更新校准。

**输出路径**：`Output/xhs/image_plan.md`

**校准流程**：

```
读取阶段3生成的 03_xhs_image_plan.md
对比最终稿（04_draft.md 或 05_draft_optimized_v2.md）：
├─ 文章标题是否变化？ → 更新封面图的痛点文案/标题
├─ 核心功能是否调整？ → 更新功能截图的展示内容
├─ 卖点是否优化？    → 更新 AI 生图提示词中的卖点描述
└─ 整体风格是否调整？ → 更新配色方案

输出校准后的完整配图策划到 Output/xhs/image_plan.md
```
...
# - 来源优先级:
#   1. 用户已在 Materials/Medias/images/ 提供 cover.{ext} → 直接使用
#   2. 未提供 → 询问用户是否 AI 生成（generate_cover.py → 保存到 Materials/）
...
| 图片来源 | 优先从 `Materials/Medias/images/cover.*` 获取；若不存在，**询问用户**是否 AI 生成 |
| AI 生成 | 使用 `generate_cover.py --workspace <工作区>`，默认中文文字，保存到 Materials/ |
...
步骤 A：检查是否已有封面图
  ...
  ├─ 存在 cover.{jpg|jpeg|png|gif|webp|bmp} 文件？
  │   ├─ 是 → 直接使用，跳到步骤 D
  │   └─ 否 → 继续步骤 B
...
步骤 C：AI 生成封面图
  根据用户选择，调用 generate_cover.py：

  # 默认：使用内置「科技视觉绘图专家」模板 + 文章内容（推荐）
  python3 ~/.cursor/skills/content-creator/scripts/generate_cover.py \
    --article "<Output/{platform}/article.md 或 _drafts/04_article_draft.md>" \
    --workspace "<工作区根目录>" \
    --size "16:9" \
    --resolution "1K"
  ...
  ⚠️ 默认规则：
  - --article 模式：内置暗黑赛博/3D渲染风格，禁止图片中出现任何文字
  - --prompt 模式：图片中文字默认中文，--lang en 切换英文
  - 生成的封面自动保存到 Materials/Medias/images/cover.jpg
  - 支持的比例: 1:1, 2:3, 3:2, 3:4, 4:3, 4:5, 5:4, 9:16, 16:9

步骤 D：复制封面到平台输出目录
  从：Materials/Medias/images/cover.{ext}

...
def expected_output_paths(output_path, total):
    return [resolve_output_path(output_path, idx, total) for idx in range(total)]
...
def submit_task_and_wait(
    ...
    if not force:
        expected_outputs = expected_output_paths(output_path, n)
        if expected_outputs and all(os.path.exists(path) for path in expected_outputs):
            print(f"Skip existing output for {output_path}: all expected files already exist.")
            return {
                "success": True,
                "files": expected_outputs,
                "output": output_path,
                "task_id": None,
                "provider": "skipped_existing",
...
def main():
    parser = argparse.ArgumentParser(description="Generate article illustrations using APIMart or OpenRouter API")
    parser.add_argument("--prompt", type=str, nargs="+", required=True, help="中文生图提示词 (支持传入多个实现并发生成)")
    ...
    parser.add_argument(
        ...
        help="图片横纵比。可传 1 个值用于全部任务，也可按 prompt 数量传入多个值一一对应。推荐使用 --aspect_ratio；--size 为兼容旧命令的别名。"
    )
    ...
    parser.add_argument("--force", action="store_true", help="强制重新生成，忽略本地缓存与已存在输出文件")
    ...
    aspect_ratios = ["16:9"] * len(args.prompt)
    if args.aspect_ratio:
        if len(args.aspect_ratio) == 1:
            aspect_ratios = [args.aspect_ratio[0]] * len(args.prompt)
        elif len(args.aspect_ratio) == len(args.prompt):
            aspect_ratios = args.aspect_ratio
        else:
            print("Error: --aspect_ratio must have exactly 1 item or match the number of --prompt arguments.")
            sys.exit(1)
    ...
    apimart_model = apimart_creds.get("model") or "gemini-3-pro-image-preview"
    ...
    print(
        ...
        f"(provider={args.provider}, apimart_model={apimart_model}, openrouter_model={openrouter_model}, "
        f"aspect_ratio={'mixed' if len(set(aspect_ratios)) > 1 else aspect_ratios[0]}, resolution={args.resolution})"
    )
    ...
    with ThreadPoolExecutor(max_workers=min(5, len(args.prompt))) as executor:
        future_to_task = {
            ...
            for prompt, output_path, ref_img, aspect_ratio in zip(args.prompt, args.output, ref_images, aspect_ratios)
        }
            ...
            try:
                ...
                if result["success"]:
                    if result.get("skipped_existing"):
                        print(f"⏭️ Skip for {output_path}: output already exists, use --force to regenerate.")
                    else:

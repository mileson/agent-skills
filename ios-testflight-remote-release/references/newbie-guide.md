...
4. 进入 `App Store Connect API`，并停留在 `Team Keys` 标签。

### Step 2: 创建 Key（如果还没有）
1. 点击 `Active` 旁边的 `+` 新建 Key。
2. 填写名称，例如 `Agent Internal Testing`。
3. `Access` 选择 `App Manager`（覆盖上传与 TestFlight 分发所需权限）。
4. 点击生成后，在对应行点击下载按钮获取 `.p8` 文件（仅下载一次）。

### Step 3: 对应三个字段（其中 `ASC_KEY_ID` 可自动推导）
1. `ASC_KEY_FILEPATH`：本机 `.p8` 文件绝对路径（例如 `/Users/you/Downloads/AuthKey_72L73DSSFC.p8`）。
2. `ASC_ISSUER_ID`：在 `Team Keys` 标签上方复制 Issuer ID（UUID）。
3. `ASC_KEY_ID`：
   - 脚本会从文件名 `AuthKey_<KEY_ID>.p8` 自动提取，用户无需单独填写；
   - 若文件名不符合该格式，请先重命名为 `AuthKey_<KEY_ID>.p8` 再执行发布。
...
# ASC_KEY_ID 由脚本从 ASC_KEY_FILEPATH 自动推导，无需手动设置
```

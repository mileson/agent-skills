...
TESTERS_CLI_SET="false"
INPUT_TESTER_EMAILS="${TESTER_EMAILS:-}"
TESTER_EMAILS_SOURCE=""
TESTER_EMAILS_CANDIDATES=""
CHANGE_TYPE="patch"
MARKETING_VERSION=""
CONFIRM_VERSION=""
IPA_PATH_INPUT="${IPA_PATH:-}"
SKIP_BUILD="false"
DRY_RUN="false"
AUTO_VERIFY="true"
...
Options:
  ...
  --change-type <build-only|patch|minor|major> 未指定版本时用于建议版本（默认 patch）
  --confirm-version <x.y.z>        确认采用某个版本（用于“先建议后确认”）
  --ipa <path>                     skip-build 时指定要上传的 ipa 路径
...
derive_key_id_from_key_filepath() {
  local key_path="$1"
  local file_name
  file_name="$(basename "${key_path}")"
  if [[ "${file_name}" =~ ^AuthKey_([A-Za-z0-9]+)\.p8$ ]]; then
    echo "${BASH_REMATCH[1]}"
  fi
}
...
resolve_fastlane_cmd() {
  if [[ -f "${PROJECT_ROOT}/Gemfile" ]] && has_cmd bundle; then
    ...
    bundle_version="$(run_cmd_version bundle exec fastlane)"
    if semver_ge "${bundle_version}" "${MIN_FASTLANE_VERSION}"; then
      FASTLANE_EFFECTIVE_CMD="bundle exec fastlane"
      FASTLANE_ACTIVE_VERSION="${bundle_version}"
      echo "bundle exec fastlane"
      return 0
    fi
    echo "[WARN] Gemfile fastlane 版本过低: ${bundle_version:-<missing>}，将改用系统 fastlane。" >&2
  fi
  ensure_fastlane_min_version >&2
  echo "${FASTLANE_EFFECTIVE_CMD}"
}
...
read_info_plist_path() {
  local ws="$1"
  local sc="$2"
  [[ -n "${ws}" && -n "${sc}" ]] || return 0
  xcodebuild -workspace "${PROJECT_ROOT}/${ws}" -scheme "${sc}" -showBuildSettings 2>/dev/null \
    | awk -F' = ' '/ INFOPLIST_FILE = / {print $2; exit}'
}

read_plist_marketing_version() {
  local plist_path="$1"
  [[ -f "${plist_path}" ]] || return 0
  /usr/libexec/PlistBuddy -c "Print :CFBundleShortVersionString" "${plist_path}" 2>/dev/null || true
}

verify_marketing_version_consistency() {
  local expected="$1"
  local ws="$2"
  local sc="$3"
  local actual
  actual="$(read_current_marketing_version "${ws}" "${sc}")"
  [[ -n "${actual}" ]] || fail "无法读取当前 MARKETING_VERSION，无法继续发布。"
  [[ "${actual}" == "${expected}" ]] || fail "版本一致性校验失败：xcodebuild 读取 MARKETING_VERSION=${actual}，期望=${expected}。请先修正项目版本后重试。"

  local plist_rel plist_abs plist_version
  plist_rel="$(read_info_plist_path "${ws}" "${sc}")"
  if [[ -n "${plist_rel}" ]]; then
    plist_abs="${PROJECT_ROOT}/${plist_rel}"
    plist_version="$(read_plist_marketing_version "${plist_abs}")"
    if [[ -n "${plist_version}" && "${plist_version}" != "\$(MARKETING_VERSION)" && "${plist_version}" != "${expected}" ]]; then
      fail "Info.plist 版本校验失败：CFBundleShortVersionString=${plist_version}，期望=${expected}。请确保与 MARKETING_VERSION 一致。"
    fi
  fi
  echo "[OK] 版本一致性校验通过：${expected}"
}

read_ipa_marketing_version() {
  local ipa_path="$1"
  [[ -f "${ipa_path}" ]] || return 0
  python3 - "${ipa_path}" <<'PY' || true
import plistlib
import sys
import zipfile
...
try:
    with zipfile.ZipFile(ipa_path, "r") as zf:
        plist_name = next((n for n in zf.namelist() if n.startswith("Payload/") and n.endswith(".app/Info.plist")), "")
        if not plist_name:
            sys.exit(0)
...
MIN_FASTLANE_VERSION="${MIN_FASTLANE_VERSION}" AUTO_FIX_FASTLANE="${AUTO_FIX_FASTLANE}" "${preflight_cmd[@]}"
...
if [[ ${resolve_rc} -ne 0 ]]; then
  fail "资料未齐全。请补齐 ASC_ISSUER_ID 与 ASC_KEY_FILEPATH；ASC_KEY_ID 会由 AuthKey_<KEY_ID>.p8 自动提取，若文件名不符合请先重命名后重试。"
fi
...
if [[ -z "${ASC_KEY_ID:-}" ]]; then
  ASC_KEY_ID="$(derive_key_id_from_key_filepath "${ASC_KEY_FILEPATH}")"
fi
[[ -n "${ASC_KEY_ID:-}" ]] || fail "无法从 ASC_KEY_FILEPATH 自动推导 ASC_KEY_ID。请将 .p8 文件名改为 AuthKey_<KEY_ID>.p8 后重试。"
[[ -n "${TESTERS}" ]] || fail "缺少 tester 邮箱，请传入 --testers 或设置 TESTER_EMAILS"
if [[ "${TESTERS_CLI_SET}" != "true" && -z "${INPUT_TESTER_EMAILS}" ]]; then
  case "${TESTER_EMAILS_SOURCE:-}" in
    local|memory)
      echo "[ACTION REQUIRED] 当前 TESTER_EMAILS 来自 ${TESTER_EMAILS_SOURCE} 自动推断，已为避免误发而中止。"
      if [[ -n "${TESTER_EMAILS_CANDIDATES:-}" ]]; then
        echo "[INFO] 检测到本地候选 tester 邮箱: ${TESTER_EMAILS_CANDIDATES}"
      fi
      echo "[INFO] 请显式确认后重试："
      echo "  --testers \"466257277@qq.com\""
      echo "  或 export TESTER_EMAILS=\"466257277@qq.com\""
      exit 21
      ;;
  esac
fi
...
if [[ -z "${MARKETING_VERSION}" ]]; then
  ...
  suggested_build_only="$(next_version "${current_version}" "build-only")"
  suggested_patch="$(next_version "${current_version}" "patch")"
  suggested_minor="$(next_version "${current_version}" "minor")"
  suggested_major="$(next_version "${current_version}" "major")"
  ...
  if [[ -z "${CONFIRM_VERSION}" ]]; then
    ...
    echo "[INFO] 可选版本建议："
    echo "  1) build-only（版本不变，仅增加 build）: ${suggested_build_only}"
    echo "  2) patch（bug/小优化）: ${suggested_patch}"
    echo "  3) minor（较大功能迭代）: ${suggested_minor}"
    echo "  4) major（大版本升级）: ${suggested_major}"
    echo "[INFO] 当前 change-type=${CHANGE_TYPE} 对应建议: ${suggested_version}"
    echo "[INFO] 请确认后重试，例如："
    ...
    echo "  # 仅增加 build，不改 MARKETING_VERSION："
    echo "  bash scripts/release_internal.sh --project-root \"${PROJECT_ROOT}\" --confirm-version \"${suggested_build_only}\" --change-type \"build-only\""
    exit 20
  fi
...
if [[ "${SKIP_BUILD}" == "true" ]]; then
  if [[ -z "${IPA_PATH_INPUT}" ]]; then
    IPA_PATH_INPUT="${PROJECT_ROOT}/build/fastlane/${SCHEME}.ipa"
  fi
  [[ -f "${IPA_PATH_INPUT}" ]] || fail "skip-build=true 但未找到 ipa：${IPA_PATH_INPUT}，请通过 --ipa 指定。"
  ipa_version="$(read_ipa_marketing_version "${IPA_PATH_INPUT}")"
  [[ -n "${ipa_version}" ]] || fail "无法读取 ipa 内 CFBundleShortVersionString：${IPA_PATH_INPUT}"
  [[ "${ipa_version}" == "${MARKETING_VERSION}" ]] || fail "skip-build 模式版本不一致：ipa=${ipa_version}，目标=${MARKETING_VERSION}。请重新构建或更换 ipa。"
fi
...
verify_marketing_version_consistency "${MARKETING_VERSION}" "${WORKSPACE}" "${SCHEME}"
...
(
  ...
  export ASC_KEY_FILEPATH="${ASC_KEY_FILEPATH}"
  export ASC_KEY_IS_BASE64="${ASC_KEY_IS_BASE64:-false}"
  ...
  if [[ "${SKIP_BUILD}" == "true" ]]; then
    export IPA_PATH="${IPA_PATH_INPUT}"
  fi
...
if [[ "${AUTO_VERIFY}" == "true" ]]; then
  ...
  ASC_KEY_FILEPATH="${ASC_KEY_FILEPATH}" \
  ASC_KEY_IS_BASE64="${ASC_KEY_IS_BASE64:-false}" \
  IOS_APP_IDENTIFIER="${APP_IDENTIFIER}" \
  TESTER_EMAILS="${TESTERS}" \
  INTERNAL_GROUP_NAME="${GROUP_NAME}" \
  MIN_FASTLANE_VERSION="${MIN_FASTLANE_VERSION}" \
  SKILL_FASTLANE_CMD="${fastlane_cmd}" \

...
FORCE="false"
DRY_RUN="false"
FASTLANE_VERSION="${FASTLANE_VERSION:-2.232.1}"
...
Options:
  ...
  --fastlane-version <x.y.z>       生成 Gemfile 时使用的 fastlane 版本（默认 2.232.1）
...
write_gemfile() {
  local version="$1"
  local gemfile_path="${PROJECT_ROOT}/Gemfile"
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --fastlane-version) FASTLANE_VERSION="${2:-}"; shift 2 ;;
...
echo "Gemfile Path        : ${PROJECT_ROOT}/Gemfile"
...
if [[ "${FORCE}" == "true" || ! -f "${PROJECT_ROOT}/Gemfile" ]]; then
  write_gemfile "${FASTLANE_VERSION}"
fi

echo "[DONE] 已创建/更新 fastlane/Appfile、fastlane/Fastfile 与 Gemfile。"
